...
각 꽃에 대해 다음 정보가 추가로 제공됩니다:
- **꽃 학명** (nameEn): 예) "Helianthus annuus"
- **꽃 색상** (color): 예) "밝은 노란색"
- **개화 시기** (bloomingSeason): 예) "7월 ~ 9월 (여름)"
- **원산지** (origin): 예) "북아메리카"
- **향기** (fragrance): 예) "거의 없음", "은은한 달콤한 향"
- **꽃말 유래** (meaningStory): 2-3줄 설명
- **재미있는 사실** (funFact): 1-2줄 설명

이 정보들은 DB의 `emotions` 테이블에 저장되며, 프론트엔드에서 하단 상세 정보로 표시됩니다.
...
#### 3. 최신 LLM의 한국어 성능
- Claude 3.5 Sonnet은 한국어 처리가 매우 우수
- 영어가 더 정확했던 건 이전 세대 모델 시절
...
- [ ] Claude API 연동 (Claude 3.5 Sonnet)
- [ ] 프롬프트 버전 관리 시스템
- [ ] DTO 및 Enum 구현
- [ ] 보안 검증 로직 구현
- [ ] Rate Limiting 설정
